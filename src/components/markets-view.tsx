...
function MarketCard({ market, now }: { market: Market; now: number }) {
  ...
  const terminal = isTerminalMarket(phase.status);
  ...
  return (
    <article className="rounded-card border border-border bg-surface p-4 sm:p-5">
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-[minmax(0,1fr)_240px] lg:items-end">
        <div>
          ...
          <p className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            {terminal
              ? `Resolved ${formatSchedule(market.resolvedAt ?? market.observationEndsAt)}`
              : `${phase.countdownLabel} ${formatSchedule(phase.status === "Scheduled" ? market.opensAt : market.locksAt)}`}
            {!terminal && phase.remainingMs > 0 && (
              <span className="font-mono tabular-nums text-foreground">{countdownFor(market, now)}</span>
            )}
          </p>
        </div>
        <div className="grid grid-cols-2 gap-2">
          {market.outcomes.map((outcome) => (
            <div className={`rounded-control p-3 ${outcome.id === market.winningOutcomeId ? "bg-accent/10" : "bg-surface-raised"}`} key={outcome.id}>
              ...
              <p className={`mt-2 font-mono text-xl font-semibold tabular-nums ${outcome.id === market.winningOutcomeId ? "text-accent" : ""}`}>{outcome.probability}%</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
        <p className="text-xs text-muted-foreground">{settings.hidePoolValues ? "Pool hidden by your settings" : `${formatCompactUsd(market.pool)} pool`}</p>
...
export function MarketsView() {
  ...
  const { data, status: loadStatus, error, retry } = useMarkets();
  ...
  const filtered = useMemo(() => {
    ...
    return (data ?? []).filter((market) => {
      const derived = marketPhase(market, now).status;
      const inView = view === "history" ? isTerminalMarket(derived) : !isTerminalMarket(derived);
  ...
  }, [data, now, view, search, category, status, remindersOnly, settings.reminders]);
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 md:px-6 lg:px-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-2 display text-4xl md:text-5xl">Follow what is live and what comes next.</h1>
        ...
        {loadStatus === "loading" && <div className="mt-6 grid gap-3" aria-live="polite">{[0, 1, 2].map((item) => <div className="h-52 animate-pulse rounded-card bg-surface" key={item} />)}</div>}
        {loadStatus === "error" && <section className="mt-6 rounded-card border border-danger/30 bg-danger/8 p-5" role="alert"><CircleAlert className="size-5 text-danger" aria-hidden="true" /><h2 className="mt-3 font-semibold">Markets did not load</h2><p className="mt-2 text-sm text-muted-foreground">{error?.message ?? "Retry the market service request."}</p><button className="button-secondary mt-4" type="button" onClick={retry}><RefreshCw className="size-4" aria-hidden="true" />Retry</button></section>}
        {loadStatus === "ready" && filtered.length === 0 && <section className="mt-6 grid min-h-64 place-items-center rounded-card border border-dashed border-border bg-surface px-6 text-center"><div><Search className="mx-auto size-8 text-muted-foreground" aria-hidden="true" /><h2 className="mt-4 font-semibold">No markets match these filters</h2><p className="mt-2 text-sm text-muted-foreground">Clear the search and filters to see the full catalogue.</p><button className="button-secondary mt-5" type="button" onClick={() => router.replace(`${pathname}?view=${view}`)}>Clear filters</button></div></section>}
        {loadStatus === "ready" && filtered.length > 0 && <section className="mt-6 grid gap-4" aria-label={view === "history" ? "Market history" : "Market schedule"}>{filtered.map((market) => <MarketCard market={market} now={now} key={market.id} />)}</section>}
      </main>
    </div>
  );
}
