...
import { useMarkets, useNotifications } from "@/hooks/use-scry";
...
export function NotificationsView() {
  ...
  const { data, status, error, retry } = useNotifications();
  ...
  const notifications = useMemo(() => {
    ...
    return [...reminderNotifications, ...(data ?? [])].sort((first, second) => second.createdAt.localeCompare(first.createdAt));
  }, [settings.reminders, markets, data]);
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 md:px-6 lg:px-8">
        ...
        {status === "loading" && <div className="mt-8 grid gap-3" aria-live="polite">{[0, 1, 2].map((item) => <div className="h-28 animate-pulse rounded-card bg-surface" key={item} />)}</div>}
        {status === "error" && <section className="mt-8 rounded-card border border-danger/30 bg-danger/8 p-5" role="alert"><CircleAlert className="size-5 text-danger" aria-hidden="true" /><h2 className="mt-3 font-semibold">Updates did not load</h2><p className="mt-2 text-sm text-muted-foreground">{error?.message ?? "Retry the notification service request."}</p><button className="button-secondary mt-4" type="button" onClick={retry}><RefreshCw className="size-4" aria-hidden="true" />Retry</button></section>}
